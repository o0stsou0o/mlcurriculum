...
def getNeighbors(trainingSet, testInstance, k, expertIndex):
    ...
    for trainingInstance in trainingSet:
        ...
        dist = distance(testDNA, curDNA, expertIndex)
...
# Returns the weights of our experts depending on how they performed at guessing
# actual labels.
def knnMain(trainingSet, testSet, experts):
    ...
    # incorrect.
    expertLength = len(experts)
    for testInstance in testSet: 
        for expertIndex in range(expertLength):
            neighbors = getNeighbors(trainingSet, testInstance, k, expertIndex)
            label = getLabel(neighbors)
            actualLabel = testInstance[1]
            if (label != actualLabel):
                expertWeights[expertIndex] *= penalty
    return experts
